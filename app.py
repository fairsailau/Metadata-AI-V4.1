...
with st.sidebar:
    ...
    if hasattr(st.session_state, "authenticated") and st.session_state.authenticated:
        ...
        if st.button("Home", use_container_width=True, key="nav_home", label_visibility="visible"):
            navigate_to("Home")
        
        if st.button("File Browser", use_container_width=True, key="nav_file_browser", label_visibility="visible"):
            navigate_to("File Browser")
        
        if st.button("Document Categorization", use_container_width=True, key="nav_doc_cat", label_visibility="visible"):
            navigate_to("Document Categorization")
            
        if st.button("Metadata Configuration", use_container_width=True, key="nav_meta_config", label_visibility="visible"):
            navigate_to("Metadata Configuration")
            
        if st.button("Process Files", use_container_width=True, key="nav_process", label_visibility="visible"):
            navigate_to("Process Files")
            
        if st.button("View Results", use_container_width=True, key="nav_view", label_visibility="visible"):
            navigate_to("View Results")
            
        if st.button("Apply Metadata", use_container_width=True, key="nav_apply", label_visibility="visible"):
        ...
        if st.button("Refresh Templates", key="refresh_templates_btn", label_visibility="visible"):
        ...
        # Logout button
        if st.button("Logout", use_container_width=True, key="nav_logout", label_visibility="visible"):
            st.session_state.authenticated = False
            st.session_state.client = None
            navigate_to("Home")
            st.rerun()
...
if not hasattr(st.session_state, "authenticated") or not st.session_state.authenticated:
    ...
    if not hasattr(st.session_state, "current_page") or st.session_state.current_page == "Home":
        ...
        with col1:
            if st.button("Browse Files", use_container_width=True, key="quick_browse", label_visibility="visible"):
                navigate_to("File Browser")
                st.rerun()
        
        with col2:
            if st.button("Categorize Documents", use_container_width=True, key="quick_categorize", label_visibility="visible"):
                navigate_to("Document Categorization")
                st.rerun()
        
        with col3:
            if st.button("Configure Metadata", use_container_width=True, key="quick_configure", label_visibility="visible"):
                navigate_to("Metadata Configuration")
                st.rerun()
